...
make_kernel <- function(cov_names,
  ...
  if (identical(type, "custom")) {
    ...
    kinfo              <- trend_help(type, do_return = TRUE)
...
get_kernels <- function() {
  ...
  delta_satlink_dim_card = list(description = paste(
                ...
                "         saturating link is applied per feature dimension: g1 = sat*tanh((q_one1-q_two1)^gamma/sat),\n",
                "         g2 = sat*tanh((q_one2-q_two2)^gamma/sat), emitted as an AllocShape [g1, g2, 0, ...]\n",
                "         stream (route g1 to the dim-1 feature accumulators, g2 to the dim-2 ones with a\n",
                "         per-dimension coding map). For win-all / win-one per-feature races. sat is chosen\n",
                "         by trial cardinality (Double -> sat_double, Single -> sat_single); sat -> Inf nests\n",
                "         linear per dimension. Requires the same kernel_args as delta_satlink_gamma_card.\n",
                "         Parameters: q0, alpha, sat_single, sat_double, gamma."
              ),
              default_pars = c("q0", "alpha", "sat_single", "sat_double", "gamma"),
              transforms = list(func = list("q0" = "plogis", "alpha" = "pnorm",
  ...
  delta_satlink_gamma_card_elemseed_2a = list(description = paste(
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  delta_noisyor_card = list(description = paste(
                "Option-level noisy-OR cardinality-saturation read-out (delta).\n",
                "         Learns one Q per elemental covariate with independent delta updates,\n",
                "         composes each option's elemental Qs as V = 1 - prod(1 - q_i), forms\n",
                "         D = V_one - V_two from the kernel_args feature-index columns, and emits\n",
                "         g = sat * tanh(D^gamma / sat) as an AllocShape [g, -g, 0, ...] stream.\n",
                "         This gives an elemental causal-composition channel without learning direct\n",
                "         compound Q states. sat is chosen by trial cardinality (Double -> sat_double,\n",
                "         Single -> sat_single); sat -> Inf nests a linear noisy-OR contrast.\n",
                "         Requires the same kernel_args as delta_satlink_gamma_card. Parameters:\n",
                "         q0, alpha, sat_single, sat_double, gamma."
              ),
              default_pars = c("q0", "alpha", "sat_single", "sat_double", "gamma"),
              transforms = list(func = list("q0" = "plogis", "alpha" = "pnorm",
                                            "sat_single" = "exp", "sat_double" = "exp",
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  rw_noisyor_card = list(description = paste(
                "Noisy-OR read-out with GRADIENT (shared-PE) learning through the OR link\n",
                "         -- the causal-power learner. Same read-out as delta_noisyor_card\n",
                "         (V = 1 - prod(1 - q) per option, D = V_one - V_two, cardinality sat,\n",
                "         AllocShape [g, -g, 0, ...]), but ONE shared prediction error\n",
                "         pe = reward - V_chosen updates every active cue scaled by its leverage\n",
                "         prod_other(1 - q_other) (= dV/dq: explaining-away -- a strong partner\n",
                "         absorbs the outcome). Converges to the TRUE COMPOUND probabilities (the\n",
                "         marginal composition overshoots); solo strengths recovered up to the\n",
                "         design's between-dimension ridge when cues never appear alone. One active\n",
                "         cue reduces to the plain delta. Q clamped to [0,1].\n",
                "         Requires the same kernel_args as delta_satlink_gamma_card. Parameters:\n",
                "         q0, alpha, sat_single, sat_double, gamma."
              ),
              default_pars = c("q0", "alpha", "sat_single", "sat_double", "gamma"),
  ...
  rw_satlink_gamma_card = list(description = paste(
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  rw_satlink_dim_card = list(description = paste(
                "Per-DIMENSION trial-type saturating satlink read-out with RESCORLA-WAGNER\n",
                "         (shared/summed PE) learning. = rw_satlink_gamma_card's learner (ONE Q store over\n",
                "         the combined {elem, cfg} cue set; the chosen option's active cues share ONE\n",
                "         PE = reward - sum(active Q)) + delta_satlink_dim_card's per-stream emission:\n",
                "         AllocShape [g(d1), g(d2), g(d_cfg), 0, ...] with d1/d2 the per-dimension\n",
                "         elemental advantages and d_cfg the compound advantage. Route d1/d2 per\n",
                "         dimension and d_cfg team-wide with ONE coding map (shared-PE learning makes\n",
                "         elemental + configural one value system -> one weight). TRIAL-TYPE saturation:\n",
                "         one sat per trial for ALL streams (feat_*_2 present -> sat_double, else\n",
                "         sat_single; the compound only exists on Double -> always sat_double). For the\n",
                "         win-all / win-one per-feature races (the ladder RW learner lesion).\n",
                "         Requires kernel_args: n_elem, feat_one_1_idx_column, feat_two_1_idx_column\n",
                "         (+ optional feat_one_2/feat_two_2/feat_one_3/feat_two_3 + q_reset_column).\n",
                "         Parameters: q0, alpha, sat_single, sat_double, gamma."
              ),
              default_pars = c("q0", "alpha", "sat_single", "sat_double", "gamma"),
              # q0 = plogis (matches rw_satlink_gamma_card / delta_plogis_q0 for by-name sharing).
              transforms = list(func = list("q0" = "plogis", "alpha" = "pnorm",
                                            "sat_single" = "exp", "sat_double" = "exp",
                                            "gamma" = "exp")),
              bases = base_2p,
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  rw_satlink_dim_card_ff = list(description = paste(
                "FULL-FEEDBACK (two per-option PEs) sibling of rw_satlink_dim_card.\n",
                "         Same per-dimension trial-type saturating emission (AllocShape\n",
                "         [g(d1), g(d2), g(d_cfg), 0, ...], routed per-dimension + team-wide with ONE\n",
                "         coding map and ONE weight) but the LEARNING does TWO per-option summed-error\n",
                "         updates per trial, as rw_satlink_gamma_card_ff: PE_one = r_one - sum(Q over\n",
                "         option-1 cues), PE_two likewise, with r_one/r_two read from each option's\n",
                "         FIRST cue's covariate column (full-feedback covariates set every present cue\n",
                "         to its OWN option's reward). For the win-all race RW learner lesion under\n",
                "         full feedback (blocking design), where the partial-feedback kernel's single\n",
                "         shared PE would conflate the two options.\n",
                "         Requires kernel_args: n_elem, feat_one_1_idx_column, feat_two_1_idx_column\n",
                "         (+ optional feat_one_2/feat_two_2/feat_one_3/feat_two_3 + q_reset_column).\n",
                "         Parameters: q0, alpha, sat_single, sat_double, gamma."
              ),
              default_pars = c("q0", "alpha", "sat_single", "sat_double", "gamma"),
              # q0 = plogis (matches rw_satlink_dim_card / delta_plogis_q0 for by-name sharing).
              transforms = list(func = list("q0" = "plogis", "alpha" = "pnorm",
                                            "sat_single" = "exp", "sat_double" = "exp",
                                            "gamma" = "exp")),
              bases = base_2p,
  ...
  rw_satlink_gamma_card_ff = list(description = paste(
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  rw_satlink_gamma_card_2ch = list(description = paste(
                "TWO-CHANNEL read-out sibling of rw_satlink_gamma_card (partial feedback).\n",
                "         LEARNING is byte-identical to rw_satlink_gamma_card (ONE shared Q store over\n",
                "         {feat_1, feat_2, compound}; one shared PE = reward - sum(active Q) -- the\n",
                "         compound STAYS in cue competition). The ONLY difference is the READ-OUT: the\n",
                "         elemental advantage (features only) and the configural advantage (compound\n",
                "         only) are emitted as SEPARATE AllocShape column pairs so each can carry its\n",
                "         OWN weight: [g_elem, -g_elem, g_cfg, -g_cfg, 0, ...]. Route cols 1-2 with\n",
                "         make_alloc_selector_map -> w_d and cols 3-4 with make_alloc_selector_map_cfg\n",
                "         -> w_d_cfg (two bases SHARING this kernel). d_elem sat by cardinality; d_cfg\n",
                "         always sat_double. Requires n_elem >= 4, feat_one_1_idx_column,\n",
                "         feat_two_1_idx_column (+ optional feat_*_2 / feat_*_3 + q_reset_column).\n",
                "         Parameters: q0, alpha, sat_single, sat_double, gamma."
              ),
              default_pars = c("q0", "alpha", "sat_single", "sat_double", "gamma"),
              transforms = list(func = list("q0" = "plogis", "alpha" = "pnorm",
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  rw_satlink_gamma_card_2ch_ff = list(description = paste(
                "FULL-FEEDBACK two-channel read-out. LEARNING byte-identical to\n",
                "         rw_satlink_gamma_card_ff (TWO per-option summed-error updates per trial; the\n",
                "         compound stays in the shared PE). Separate elemental / configural advantage\n",
                "         column pairs as rw_satlink_gamma_card_2ch. Requires n_elem >= 4,\n",
                "         feat_one_1_idx_column, feat_two_1_idx_column (+ optional feat_*_2 / feat_*_3\n",
                "         + q_reset_column). Parameters: q0, alpha, sat_single, sat_double, gamma."
              ),
              default_pars = c("q0", "alpha", "sat_single", "sat_double", "gamma"),
              transforms = list(func = list("q0" = "plogis", "alpha" = "pnorm",
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  rw_satlink_gamma_card_ph = list(description = paste(
                "PEARCE-HALL Rescorla-Wagner cardinality-saturation satlink read-out.\n",
                "         = rw_satlink_gamma_card but the FIXED learning rate is replaced by a\n",
                "         per-cue PEARCE-HALL associability (dynamic, surprise-driven rate):\n",
                "         shared compound PE = reward - sum(active Q); each active cue updates at\n",
                "         its current associability and that associability relaxes toward |PE|\n",
                "         (assoc <- eta*|PE| + (1-eta)*assoc, clamped to [0,1] since it IS the LR).\n",
                "         Same cardsat read-out / optional 3rd cue / cardinality rule as\n",
                "         rw_satlink_gamma_card. Requires kernel_args: n_elem, feat_one_1_idx_column,\n",
                "         feat_two_1_idx_column (+ optional feat_one_2/feat_two_2/feat_one_3/feat_two_3\n",
                "         + q_reset_column). Parameters: q0, alpha0 (initial associability), eta\n",
                "         (associability mixing weight), sat_single, sat_double, gamma."
              ),
              ...
              n_outputs    = 1L,
              NA_allowed=TRUE),
  rw_satlink_gamma_card_expdecr = list(description = paste(
                "EXP-DECREASING-rate Rescorla-Wagner cardinality-saturation satlink read-out.\n",
                "         = rw_satlink_gamma_card + the delta_expdecr schedule: the FIXED alpha is\n",
                "         replaced by alpha_eff(t) = Phi(alpha_base + alpha_w*exp(-d_alpha_ed*block_trial)),\n",
                "         alpha_base on the probit scale; alpha_w, d_alpha_ed >= 0 (exp); block_trial =\n",
                "         within-block 0-based exposure count. The shared compound PE updates all active\n",
                "         cues at alpha_eff(t). Same cardsat read-out / optional 3rd cue / cardinality\n",
                "         rule as rw_satlink_gamma_card. d_alpha_ed -> 0 or alpha_w -> 0 nests it.\n",
                "         Requires kernel_args: n_elem, feat_one_1_idx_column, feat_two_1_idx_column,\n",
                "         block_trial_column (+ optional feat_one_2/feat_two_2/feat_one_3/feat_two_3 +\n",
                "         q_reset_column). Parameters: q0, alpha_base, alpha_w, d_alpha_ed, sat_single,\n",
                "         sat_double, gamma."
              ),
              default_pars = c("q0", "alpha_base", "alpha_w", "d_alpha_ed",
                               "sat_single", "sat_double", "gamma"),
              # q0 = plogis; alpha_base = identity (probit, kernel applies Phi); alpha_w, d_alpha_ed = exp.
              transforms = list(func = list("q0" = "plogis", "alpha_base" = "identity",
                                            "alpha_w" = "exp", "d_alpha_ed" = "exp",
                                            "sat_single" = "exp", "sat_double" = "exp",
                                            "gamma" = "exp")),
...
make_data_unconditional <- function(data, pars, design, model,
  ...
  dadm_full <- design_model(
    add_accumulators(data, design$matchfun, simulate = FALSE,
                     ...
                     custom_expand = model_list$expand_accumulators),
